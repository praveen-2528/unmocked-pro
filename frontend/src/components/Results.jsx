...
export default function Results({ scoreData, testData, gameMode, liveStats, currentUser, testSessionId, answers, statusMap, timeSpentMap, reviewUserName, onReviewAnswers }) {
  ...
  const renderMultiplayerResults = () => {
    ...
    return (
      <div className="animate-fade-in" style={{ width: '100%', maxWidth: '900px' }}>
        <div className="results-header" style={{ padding: '24px 32px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '40px' }}>
          <div>
            <h1 style={{ fontSize: '2.5rem', marginBottom: '8px' }}>Test Results</h1>
            <p style={{ color: 'var(--text-secondary)', fontSize: '1.1rem' }}>
              {testData?.blueprint?.exam_name || 'General Test'} • {gameMode}
            </p>
          </div>
          <div className="avatar-placeholder" style={{ width: '64px', height: '64px', fontSize: '1.5rem', boxShadow: '0 4px 12px rgba(99, 102, 241, 0.3)' }}>
            {reviewUserName ? reviewUserName.charAt(0).toUpperCase() : (currentUser?.name ? currentUser.name.charAt(0).toUpperCase() : 'U')}
          </div>
        </div>
